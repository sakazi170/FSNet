...
class ProgressiveFrequencyFusion_Alt1(nn.Module):
    """Progressive fusion: (T1 + T1ce) → + FLAIR → + T2"""

    def __init__(self, target_channels, num_freq_bands=3):
        super(ProgressiveFrequencyFusion_Alt1, self).__init__()

        # Stage 1: T1 + T1ce (separate modules)
        self.stage1_t1_freq = FrequencyDecomposition(target_channels, num_freq_bands)
        ...
        # Stage 2: Add FLAIR (separate modules)
        self.stage2_flair_freq = FrequencyDecomposition(target_channels, num_freq_bands)
        ...
        # Stage 3: Add T2 (separate modules)
        self.stage3_t2_freq = FrequencyDecomposition(target_channels, num_freq_bands)
    ...
    def forward(self, t1_feat, t1ce_feat, flair_feat, t2_feat):
        # Stage 1: T1 + T1ce frequency complementarity
        t1_freq_components = self.stage1_t1_freq(t1_feat)
        t1ce_freq_components = self.stage1_t1ce_freq(t1ce_feat)

        # Combine frequency components (2 modalities * 3 freq_bands = 6 tensors)
        all_freq_components = []
        for t1_comp, t1ce_comp in zip(t1_freq_components, t1ce_freq_components):
            all_freq_components.extend([t1_comp, t1ce_comp])

        stage1_input = torch.cat(all_freq_components, dim=1)  # 6 * 16 = 96 channels
        stage1_fused = self.stage1_fusion(stage1_input)

        # Stage 2: Add FLAIR
        flair_freq_components = self.stage2_flair_freq(flair_feat)
        stage2_complement = self.stage2_complementarity(stage1_fused, flair_freq_components)
        stage2_input = torch.cat([stage1_fused, stage2_complement], dim=1)
        stage2_fused = self.stage2_fusion(stage2_input)

        # Stage 3: Add T2
        t2_freq_components = self.stage3_t2_freq(t2_feat)
        stage3_complement = self.stage3_complementarity(stage2_fused, t2_freq_components)
...
class PFMF1(nn.Module):
    """
    Alternative 1: Progressive Frequency-inspired Modality Fusion
    Fusion Order: (T1 + T1ce) → + FLAIR → + T2
    ...
    def __init__(self, target_channels=16):
        ...
        # Progressive frequency fusion: (T1 + T1ce) → + FLAIR → + T2
        self.progressive_freq_fusion = ProgressiveFrequencyFusion_Alt1(target_channels)
    ...
    def forward(self, I_flair, I_t2, I_t1, I_t1c):
        ...
        # Progressive frequency-domain fusion: (T1 + T1ce) → + FLAIR → + T2
        fused_features = self.progressive_freq_fusion(
            t1_features, t1ce_features, flair_features, t2_features
        )
